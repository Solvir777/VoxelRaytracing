...
impl Player {
    ...
    pub fn pan(&mut self, input_state: &InputState, settings: &Settings) {
        ...
        self.rotation.pitch =
            (self.rotation.pitch + input_state.mouse.delta_y as f32 * sensitivity).clamp(-1.7, 1.7);
    }
}
...
impl Rotation {
    fn to_rotation_matrix(&self) -> nalgebra::Rotation<f32, 3> {
        Rotation3::from_scaled_axis(Vector3::new(0., self.yaw, 0.))
            * Rotation3::from_scaled_axis(Vector3::new(self.pitch, 0., 0.))
    }
